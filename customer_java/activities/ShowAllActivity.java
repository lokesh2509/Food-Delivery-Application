...
public class ShowAllActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (type != null && type.equalsIgnoreCase("Veg")){
            firestore.collection("ShowAll").whereEqualTo("type","Veg").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                @Override
                public void onComplete(@NonNull Task<QuerySnapshot> task) {
                    if (task.isSuccessful()){
                        for (DocumentSnapshot documentSnapshot :task.getResult().getDocuments()){
        ...
        if (type != null && type.equalsIgnoreCase("Burger")){
            firestore.collection("ShowAll").whereEqualTo("type","Burger").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                @Override
                public void onComplete(@NonNull Task<QuerySnapshot> task) {
                    if (task.isSuccessful()){
                        for (DocumentSnapshot documentSnapshot :task.getResult().getDocuments()){
        ...
        if (type != null && type.equalsIgnoreCase("Non-veg")){
            firestore.collection("ShowAll").whereEqualTo("type","Non-veg").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                @Override
                public void onComplete(@NonNull Task<QuerySnapshot> task) {
                    if (task.isSuccessful()){
                        for (DocumentSnapshot documentSnapshot :task.getResult().getDocuments()){
